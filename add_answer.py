...
for item in tqdm(mquake):
    ...
    for single_hop, triple, triple_labeled in zip_item:
        ...
        single_hop[f"{short_name}_answer"] = generate_ans
        single_hop[f"{short_name}_identical"] = generate_ans in all_answer
    ...
    inputs = tokenizer(prompt, return_tensors="pt", padding=True).to(hf_model.device)
    generate_ids = hf_model.generate(**inputs, max_new_tokens=20, pad_token_id=tokenizer.eos_token_id,stopping_criteria=stopping_criteria)
    generate_str = tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
    generate_ans = generate_str.strip().split('\n')[-1].split("A: ")[-1]

    item[f'{short_name}_answer'] = generate_ans
    item[f'{short_name}_identical'] = generate_ans in all_answer
...
with open(f"preprocess/datasets/MQuAKE-CF-3k-{short_name}.json", 'w') as f:
