...
overlap_graph = circuits_Union(clean_graph, g1, g2)



print(f'Overlap Edges Num: {overlap_graph.count_included_edges()} | Overlap Nodes Num: {overlap_graph.count_included_nodes()}')
print(f'g1 Edges Num: {g1.count_included_edges()} | g1 Nodes Num: {g1.count_included_nodes()}')

print("[-------------------------Overlap Performance-------------------------------]")

logits = get_circuit_logits(model, overlap_graph, data_1)
get_component_logits(logits, model, answer_token=model.to_tokens(' Euro', prepend_bos=False)[0], top_k=5)


logits = get_circuit_logits(model, overlap_graph, data_2)
get_component_logits(logits, model, answer_token=model.to_tokens(' Euro', prepend_bos=False)[0], top_k=5)
