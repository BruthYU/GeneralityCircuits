...
def nll_loss_diff(clean_logits: torch.Tensor,
                  labels: torch.Tensor, mean=True, prob=False, loss=False, shift=True):
    if shift:
        labels = labels[..., 1:].contiguous()
        clean_logits = clean_logits[..., :-1, :].contiguous()

    logprobs = F.log_softmax(clean_logits, dim=-1)

    good = F.nll_loss(
        logprobs.view(-1, logprobs.size(-1)), labels[0].view(-1), reduction="none"
    ).view(logprobs.size()[:-1])

    bad = F.nll_loss(
        logprobs.view(-1, logprobs.size(-1)), labels[1].view(-1), reduction="none"
    ).view(logprobs.size()[:-1])
