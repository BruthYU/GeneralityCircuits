...
class Graph:
    ...
    def recover_nodes_from_edges(self):
        for edge in self.edges.values():
            if edge.in_graph:
                edge.parent.in_graph = True
                edge.child.in_graph = True
    ...
    def keys_equal(self, graph):
        return (set(self.nodes.keys()) == set(graph.nodes.keys())) and (
                    set(self.edges.keys()) == set(graph.edges.keys()))
...
def circuits_Intersection_Relax(clean_graph: Graph, graph_A: Graph, graph_B: Graph,
                                ratio=0.2, absolute = True):
    assert clean_graph.keys_equal(graph_A) and clean_graph.keys_equal(graph_B), \
        'Graphs are generated from two different models. No overlapping nodes or edges are returned.'
    for node in clean_graph.nodes.values():
        node.in_graph = False

    a = abs if absolute else lambda x: x
    half_true_edges = []
    for name, edge in graph_A.edges.items():
        if edge.in_graph and graph_B.edges[name].in_graph:
            clean_graph.edges[name].in_graph = True
        elif edge.in_graph or graph_B.edges[name].in_graph:
            edge_flag = "g_A" if edge.in_graph else "g_B"
            info = {'name':name, 'flag':edge_flag, 'abs_score':max(a(edge.score), a(graph_B.edges[name].score))}
            half_true_edges.append(info)
            clean_graph.edges[name].in_graph = False
        else:
            clean_graph.edges[name].in_graph = False

    sorted_half_true = sorted(half_true_edges, key = lambda x: x['abs_score'], reverse=True)
    relax_num = math.floor(len(sorted_half_true)*ratio)

    ga_num = 0
    gb_num = 0

    for info in sorted_half_true[:relax_num]:
        clean_graph.edges[info['name']].in_graph = True
        if info['flag'] == 'ga':
            ga_num +=1
        else:
            gb_num+=1

    print(f"Relax {ga_num} edges from Graph_A | Relax {gb_num} edges from Graph_B")
...
def circuits_Intersection(clean_graph: Graph, graph_A: Graph, graph_B: Graph):
    assert clean_graph.keys_equal(graph_A) and clean_graph.keys_equal(graph_B), \
        'Graphs are generated from two different models. No overlapping nodes or edges are returned.'
    ...
    for name, edge in graph_A.edges.items():
        if edge.in_graph == graph_B.edges[name].in_graph == True:
            clean_graph.edges[name].in_graph = True
        else:
            clean_graph.edges[name].in_graph = False
...
def circuits_Union(clean_graph: Graph, graph_A: Graph, graph_B: Graph):
    assert clean_graph.keys_equal(graph_A) and clean_graph.keys_equal(graph_B), \
        'Graphs are generated from two different models. No overlapping nodes or edges are returned.'
    ...
    for name, edge in graph_A.edges.items():
        if edge.in_graph == graph_B.edges[name].in_graph == False:
            clean_graph.edges[name].in_graph = False
        else:
            clean_graph.edges[name].in_graph = True
