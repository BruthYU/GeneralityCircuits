...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size: int, n_pos: int, scores: torch.Tensor,
                            detach=True):
    ...
    """
    activation_difference = torch.zeros((batch_size, n_pos, graph.n_forward, model.cfg.d_model), device='cuda',
...
#TODO n_pos_sentence n_pos_input
def new_make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , input_feats:dict, scores: torch.Tensor, detach=True):
    """Makes a matrix containing activation differences, and hooks to fill it and the score matrix up
    ...
    Returns:
        ...
        The third set of hooks will take in the gradients during the backwards pass and multiply it by the activation differences, adding this value in-place to the scores matrix that you passed in. 
    """
    n_pos, sentence_n_pos, label_n_pos = input_feats["n_pos"], input_feats["sentence_n_pos"], input_feats["label_n_pos"]
    ...
    def gradient_hook(prev_index: Union[slice, int], bwd_index: Union[slice, int], gradients:torch.Tensor, hook):
        ...
        grads = gradients.detach()
        # print(
        #     f"bwd_index: {bwd_index}, grads.shape: {grads.shape}")
        try:
            if grads.ndim == 3:
                grads = grads.unsqueeze(2)
            s = einsum(activation_difference[:, :sentence_n_pos, :prev_index], grads[:, :sentence_n_pos], 'batch pos forward hidden, batch pos backward hidden -> forward backward')
            s = s.squeeze(1)#.to(scores.device)
            scores[:prev_index, bwd_index] += s
            # print(
            #     f"bwd_index: {bwd_index}, activation_difference[:, :, :prev_index].shape: {activation_difference[:, :, :prev_index].shape}, grads.shape: {grads.shape}")
            # print(f"prev_index: {prev_index}, bwd_index: {bwd_index}, s.shape: {s.shape}, scores[:prev_index, bwd_index].shape: {scores[:prev_index, bwd_index].shape}")
        except RuntimeError as e:
            print("Gradient Hook Error", hook.name, activation_difference.size(), grads.size(), prev_index, bwd_index)
            raise e
...
def tokenize_plus(model: HookedTransformer, inputs: List[str], padding_side = 'right'):
    """
...
def process_input_feats(model: HookedTransformer, sentences, labels):
    ...
    # num_prompt_toks = [int((i != tokenizer.pad_token_id).sum()) for i in sentences_tokens]

    #label_tokens
    label_tokens, label_attention_masks, _, label_n_pos = tokenize_plus(model, labels)
    label_tokens = label_tokens[:, 1:]
    label_attention_masks = label_attention_masks[:, 1:]
    label_n_pos = label_n_pos - 1


    # features for hood forward and backward
    input_tokens = torch.cat((sentence_tokens, label_tokens), 1)
    forward_attention_paddings = torch.zeros(label_tokens.size(), dtype=torch.long, device='cuda')
    forward_attention_masks = torch.cat((sentence_attention_masks, forward_attention_paddings), 1)

    backward_attention_masks = torch.cat((sentence_attention_masks, label_attention_masks), 1)


    # features for loss function
    label_ids = input_tokens.clone()


    num_pad_toks = [int((i == tokenizer.pad_token_id).sum()) for i in input_tokens]

    for i in range(len(sentences)):
        label_ids[i][:sentence_n_pos] = -100
        # left padding
    label_ids[input_tokens == tokenizer.pad_token_id] = -100
    final_feats = {
        'input_tokens': input_tokens,
        'forward_attention_masks': backward_attention_masks,
        'backward_attention_masks': backward_attention_masks,
        'label_ids': label_ids,
        'sentence_n_pos': sentence_n_pos,
        'label_n_pos': label_n_pos,
        'n_pos': sentence_n_pos + label_n_pos
    }
    return final_feats
...
def get_scores_eap_ig_tokens(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric, steps=30, quiet=False):
    scores = torch.zeros((graph.n_forward, graph.n_backward), device='cuda', dtype=model.cfg.dtype)
    sentences, labels = dataloader

    input_feats = process_input_feats(model, sentences, labels)

    (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks), activation_difference = new_make_hooks_and_matrices(model, graph, 1, input_feats, scores)

    with torch.inference_mode():
        with model.hooks(fwd_hooks=fwd_hooks_corrupted):
            _ = model(input_feats['input_tokens'][1], attention_mask=input_feats['forward_attention_masks'][1].unsqueeze(0))

        input_activations_corrupted = activation_difference[:, :, graph.forward_index(graph.nodes['input'])].clone()

        with model.hooks(fwd_hooks=fwd_hooks_clean):
            _ = model(input_feats['input_tokens'][0], attention_mask=input_feats['forward_attention_masks'][0].unsqueeze(0))
    ...
    for step in range(1, steps+1):
        ...
        with model.hooks(fwd_hooks=[(graph.nodes['input'].out_hook, input_interpolation_hook(step))], bwd_hooks=bwd_hooks):
            ...
            # corrupted_logits = model(input_feats['input_tokens'][1], attention_mask=attention_mask[1])
    ...
    # '''
    # toks: tokenized sentences
    # inputs_ids: tokenized inputs
    # label_ids: padded and tokenized labels len(input_ids)==len(labels)
    # '''
    ...
    #
    # for i in range(len(sentences)):
    #     label_ids[i][num_pad_toks[i]:num_pad_toks[i] + num_prompt_toks[i]] = -100
    #     # left padding
    # label_ids[input_ids == tokenizer.pad_token_id] = -100
...
def attribute(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], aggregation='sum', method: Union[Literal['EAP', 'EAP-IG', 'EAP-IG-partial-activations', 'EAP-IG-activations', 'clean-corrupted']]='EAP-IG', ig_steps: Optional[int]=5, quiet=False):
    ...
    elif method == 'EAP-IG-tokens':
        scores = get_scores_eap_ig_tokens(model, graph, dataloader, metric, steps=ig_steps, quiet=quiet)
    else:
